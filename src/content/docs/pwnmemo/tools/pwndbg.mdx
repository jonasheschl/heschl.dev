...
<Aside type="caution">
  `patch`'s second argument is currently always interpreted as assembly instructions and converted to machine code.
  This might change in the future. For now, when you want to patch using manually selected values, use `set` instead of
  `patch`.
</Aside>
...
<Aside type="caution">
  `x` will always try to dereference a register!
</Aside>
