...
Some password checking and similar binaries (e.g. a binary accepting a value as input and returning weather or not that
...
<Aside type="caution">
  The same technique works by using `perf` or similar tools. However, this introduces a lot of kernel (scheduling, etc)
  noise into the measurements. So, when possible, use instruction counting instead.
</Aside>
