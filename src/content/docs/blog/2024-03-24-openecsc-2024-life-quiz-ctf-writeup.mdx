...
an image `/prizes/flag.jpg`. While `/prizes/flag.jpg` is never as is in the code, there is one file that comes pretty
close. Namely `get_prize.php`.
...
`session_start()` is called. This PHP standard library function checks the `PHP_SESSID` cookie for the connecting users
current session and initializes corresponding session data if present. If another session for that user is currently
being run, `session_start()` will wait until that session has completed before resuming execution.  If we could bypass
the session locking and exploit that race condition we could get ourselves 15 points.
...
Big shout out to the author, whoever you are :)
